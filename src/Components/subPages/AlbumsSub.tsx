...
const AlbumsSub = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await SONGS("denver", 5);
      if (res?.data) {
  ...
  return (
    <section>
      <div className="flex justify-between">
        ...
        <Link to="/albums" className="text-white text-lg tracking-wide cursor-pointer opacity-70 hover:opacity-100">{t('showall')}</Link>
      </div>
