...
const Sidebar = ({ selectedSongs }: siderbarProps) => {
  ...
  return (
    <section className={`fixed top-20 z-10 bg-gray-500/10 rounded-r-2xl  h-10/12 overflow-y-auto Scroll transition-all ease-in-out max-sm:w-0  ${extend ? "px-10 w-3/12  bg-gray-900/95 " : "justify-center"}`}>
      <div className="cursor-pointer py-2" onClick={() => setExtend(!extend)}>
        <figure className={`text-white flex gap-2 justify-center text-center  px-2 rounded-2xl transition-all ease-in-out ${!extend ? "flex-col":"flex-row"}`}>
          ...
          {extend && <figcaption className="truncate text-[20px] hover:bg-purple-500/20 p-1">{t('library')}</figcaption>}
